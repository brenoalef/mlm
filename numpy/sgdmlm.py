...
class SGDMLM:
    def __init__(self, learning_rate=0.01, batch_size=1, training_iters=20, intercept=False):
    ...
    def train(self, X, Y, k=None):
        ...
        for i in range(self.training_iters):
            ...
            for j in range(Dx.shape[0]):
                ...
                loss = (1.0/k) *np.sum(np.square(error))
...
def main():
    ...
    cost = sgdmlm.train(X_train, Y_train, int(round(len(X_train)*0.5)))
    Y_hat = sgdmlm.predict(test[:, 0:9])
